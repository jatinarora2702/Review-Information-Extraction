...
flags.DEFINE_string('max_max_epoch', 20, 'No. of times the LSTM cell state and value are reinitialized and trained on a stream of input')
flags = flags.FLAGS
...
def indexdata(data):
	...
	idata = list()
	for sent in data: 
		newsent = [np.array([getindex(s[0], vocabword), getindex(s[1], vocabpos), getindex(s[2], vocabattr), 0]) for s in sent]
		newsent = [np.array([0, 0, 0, 1])] * (30 - len(sent)) + newsent
...
def makemodel(is_training):
	X = tf.placeholder(tf.int32, [flags.batchsize, flags.numsteps, 4], name='input')
	...
	e_pad = tf.reshape(tf.to_float(X[:, :, 3]), (flags.batchsize, flags.numsteps, 1))
	e = tf.concat([e_w, e_p, e_pad], 2)
...
def get_labels(x, probx):
	...
	return np.concatenate((x[:, :, :-1], labeledx), axis=2)


def myfileprint(s):
	if not os.path.exists('../results/model4/data1/'): os.makedirs('../results/model4/data1/')
	f = codecs.open('../results/model4/data1/training-log.txt', 'a', 'utf-8')
	f.write(s + '\n')
	f.close()
...
def runepoch(sess, data, modeldict, fetches, epoch_no, verbose, record):
	...
	for curr, x in enumerate(data_iterator(data, flags.batchsize)):
		...
		if record: 
			...
			f = codecs.open('../results/model4/data1/record-10.txt', 'a', 'utf-8')
			newlist = newx.tolist()
			for sent in newlist:
...
def train(traindata):
	with tf.Session() as sess:
		...
		tf.train.write_graph(sess.graph_def, '../modelparams/model4/data1/', 'model.pb', as_text=False)
		sess.run(tf.global_variables_initializer())
		# saver.restore(sess, '../modelparams/model4/data1/checkpoint-9')
		for i in range(flags.max_max_epoch):
			runepoch(sess, traindata, m_train, fetches, i, True, False)
			save_path = saver.save(sess, '../modelparams/model4/data1/model')
			save_path = saver.save(sess, '../modelparams/model4/data1/checkpoint-' + str(i))
...
def test(testdata):
	with tf.Session() as sess:
		...
		saver = tf.train.Saver(max_to_keep=50)
		saver.restore(sess, '../modelparams/model4/data1/model')
		# saver.restore(sess, '../modelparams/model4/data1/checkpoint-9')
