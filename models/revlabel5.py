...
flags.DEFINE_string('wemb_path', '../datasets/data1/wordemb-text8-100.npy', 'Embeddings file')
...
def myfileprint(s):
	if not os.path.exists('../results/model5/data1/'): os.makedirs('../results/model5/data1/')
	f = codecs.open('../results/model5/data1/training-log.txt', 'a', 'utf-8')
	f.write(s + '\n')
	f.close()
...
def runepoch(sess, data, modeldict, fetches, epoch_no, verbose, record):
	...
	for curr, x in enumerate(data_iterator(data, flags.batchsize)):
		...
		if record: 
			...
			f = codecs.open('../results/model5/data1/record-10.txt', 'a', 'utf-8')
			newlist = newx.tolist()
			for sent in newlist:
...
def train(traindata):
	with tf.Session() as sess:
		...
		tf.train.write_graph(sess.graph_def, '../modelparams/model5/data1/', 'model.pb', as_text=False)
		...
		for i in range(flags.max_max_epoch):
			...
			save_path = saver.save(sess, '../modelparams/model5/data1/model')
			save_path = saver.save(sess, '../modelparams/model5/data1/checkpoint-' + str(i))
...
def test(testdata):
	with tf.Session() as sess:
		...
		saver = tf.train.Saver(max_to_keep=50)
		saver.restore(sess, '../modelparams/model5/data1/model')
